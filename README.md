...
安装结束会打印 **DSH 插件填写卡**（服务地址 / 盘古凭据，只显示一次）。
填进 DSH 设置页 →「盘古记忆系统」即可——**设置页只有一个凭据字段「盘古凭据」**，
记忆与管理面板都用它（安装脚本已让服务端两个密钥同值）。
...
脚本结束会打印 **DSH 插件填写卡**（服务地址 / 盘古凭据），只显示一次，请立即保存。

> 💡 **设置页只有一个凭据字段**：安装脚本让 `~/.pangu/.api_key` 与
> `~/.pangu/.admin_secret` 取**同一个值**，插件在管理面直接复用「盘古凭据」，
> 所以只需填一处。
> （服务端仍是两套独立校验——数据面走 `X-API-Key`、管理面走 `X-Admin-Key`；
> 共用只是安装期的取值策略。若某台服务端两把确实不同，插件管理面会 401，
> 安装脚本的填写卡会直接给出「在服务端统一」的那一行命令。）
...
> 缓存自带**指纹校验**：换了嵌入模型或维度，旧缓存会自动整体作废，
