...
把它填进 DSH 设置页 →「盘古记忆系统」即可；**盘古凭据与管理密钥默认是同一把**，
两处填同一串（详见下方「云端部署」里的说明）。
...
> 💡 **盘古凭据与管理密钥默认是同一把**：安装脚本生成**同一个值**写进
> `~/.pangu/.api_key` 与 `~/.pangu/.admin_secret`，所以设置页两处填**同一串**即可。
> （服务端仍是两套独立校验——数据面走 `X-API-Key`、管理面走 `X-Admin-Key`；
> 共用只是安装期的取值策略，日后想拆开也可以。）
