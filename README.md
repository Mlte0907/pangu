...
安装结束会打印 **DSH 插件填写卡**（服务地址 / 盘古凭据 / 管理密钥，只显示一次）。
填进 DSH 设置页 →「盘古记忆系统」即可——**设置页只需填「盘古凭据」一处**：
安装脚本默认让「管理密钥」取同一个值，插件在管理密钥留空时会自动用它
（详见下方「云端部署」里的说明）。
...
> 💡 **设置页只需填「盘古凭据」一处**：安装脚本让 `~/.pangu/.api_key` 与
> `~/.pangu/.admin_secret` 取**同一个值**，插件在「管理密钥」留空时会自动使用
> 「盘古凭据」，所以不必填两次。
> （服务端仍是两套独立校验——数据面走 `X-API-Key`、管理面走 `X-Admin-Key`；
> 共用只是安装期的取值策略。老部署若两把不同，把「管理密钥」单独填上即可。）
