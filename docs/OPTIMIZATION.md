...
| 项 | 状态 | 说明 |
| --- | --- | --- |
| **2.4 文件拆分** | ⏳ 未做 | `cli.py` 2319 行、`handlers/advanced.py` 2129 行、`core/llm.py` 1415 行、`api/server.py` 1185 行（合计 7048 行）。纯结构调整，风险与收益需单独评估，**建议单独排期** |
| **2.3 覆盖率口径** | ⏳ 未做 | `handlers/advanced.py` 36.9%，是最大单点缺口 |
| **2.2 VectorIndex 接入** | ⏸ 有意推迟 | 见 v0.2.x+ 行：等记忆量上千再评估 |
