...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function ensureStyles() {
      ...
      el.textContent = `
/* 阿里巴巴普惠体 3.0（全字符集，font-display:swap 首帧系统字体兜底） */
@font-face{font-family:"Alibaba PuHuiTi 3.0";src:url("https://static.cdn.sunmi.com/assets/fonts/Alibaba-PuHuiTi-Regular.ttf") format("truetype");font-weight:400;font-display:swap}
@font-face{font-family:"Alibaba PuHuiTi 3.0";src:url("https://static.cdn.sunmi.com/assets/fonts/Alibaba-PuHuiTi-Medium.ttf") format("truetype");font-weight:500 600;font-display:swap}
    ...
    function PanguSettings() {
      ...
      const save = async () => {
        ...
        try {
          ...
          // 多模态：开关联动工具暴露面（详见 index.js 注释）
          const mmChanged = draft.multimodal !== (config?.multimodal_enabled === true)
          if (mmChanged) patch.multimodal_enabled = draft.multimodal
          // 管理密钥：留空 = 不变（明文永不回显）
      ...
      const doRekey = async (room) => {
        ...
        try {
          await callRemote('panguAdminKeys', 'rekeyRoom', { room })
          load()
        } catch (_) {}
      }

      if (loadErr) return h('div', { style: { padding: 20 } }, h('div', { style: { height: 220 } }, h(ErrorState, { text: '配置读取失败(' + loadErr + ')', onRetry: load })))
      ...
      const isCloud = (draft?.pb || '').includes('http') && !(draft?.pb || '').includes('127.0.0.1') && !(draft?.pb || '').includes('localhost')
      // 状态卡数据
      const stat = null // 服务状态来自标签页数据面；设置页只显示连接态
      const mono = 'ui-monospace,SFMono-Regular,Menlo,monospace'

      const SecHead = (num, title, hint) => h('div', { style: { display: 'flex', alignItems: 'baseline', gap: 9, padding: '16px 0 7px', borderBottom: `1px solid ${css.borderSoft}`, marginTop: num === '00' ? 12 : 4 } },
        h('span', { style: { fontFamily: mono, fontSize: 10, fontWeight: 600, color: ACCENT } }, num),
        h('span', { style: { fontSize: 12.5, fontWeight: 600 } }, title),
        hint && h('span', { style: { fontSize: 10.5, color: css.t3, marginLeft: 'auto' } }, hint),
      )

      const Field = (t, badge, badgeColor, hint, body, desc) => h('div', { style: { padding: '12px 0', borderBottom: `1px solid ${css.borderSoft}` } },
        h('div', { style: { display: 'flex', alignItems: 'baseline', gap: 8, marginBottom: 7 } },
          h('span', { style: { fontSize: 12.5, fontWeight: 600 } }, t),
          badge && h('span', { style: { fontSize: 9, padding: '1px 7px', borderRadius: 999, fontWeight: 600, background: withAlpha(badgeColor || css.ok, '1c'), color: badgeColor || css.ok } }, badge),
          hint && h('span', { style: { fontSize: 10.5, color: css.t3, marginLeft: 'auto' } }, hint),
        ),
        body,
        desc && h('div', { style: { fontSize: 10.5, color: css.t3, marginTop: 6, lineHeight: 1.6 } }, desc),
      )

      if (!draft) return h('div', { style: { marginTop: 12 } }, [0, 1, 2, 3].map((i) => h(Skeleton, { key: i, w: '100%', h: 40, r: 8, style: { marginBottom: 8 } })))

      return h('div', { style: { padding: '16px 18px 18px', color: css.t1, animation: 'panguFade .25s ease' } },
        // 页头 + 状态卡
        h('div', { style: { fontSize: 15, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 8 } },
          h('span', { style: { width: 14, height: 14, borderRadius: 5, background: `linear-gradient(135deg, ${ACCENT}, ${ACCENT_DEEP})`, display: 'inline-flex', alignItems: 'center', justifyContent: 'center' } },
            h(Icon, { name: 'box', size: 9, color: '#fff' })),
          '盘古记忆系统',
        ),
        h('div', { style: { fontSize: 11, color: css.t3, margin: '3px 0 10px', lineHeight: 1.6 } },
          '设置写入 ', h('span', { style: { fontFamily: mono, fontSize: 10.5 } }, '~/.pangu/config.json'),
          '，保存即热加载生效 · 凭据只存本机（权限 600）永不回传界面'),
        h('div', { style: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 1, background: css.borderSoft, border: `1px solid ${css.borderSoft}`, borderRadius: 10, overflow: 'hidden' } },
          h('div', { style: { background: css.bg1, padding: '9px 12px' } },
            h('div', { style: { display: 'flex', alignItems: 'center', gap: 6, fontFamily: mono, fontSize: 12.5, fontWeight: 600 } },
              h('span', { style: { width: 7, height: 7, borderRadius: '50%', background: css.ok, animation: 'panguPulse 2.6s ease-in-out infinite' } }),
              isCloud ? '云端在线' : '本机在线'),
        ...
        // ── 00 服务与凭据 ──
        SecHead('00', '服务与凭据', isCloud ? '云端模式' : '本机模式'),
        Field('盘古 MCP / API 地址', isCloud ? '云端模式' : '本机模式', isCloud ? css.info : css.ok, '保存即对 REST/WS 生效',
          h('input', { className: 'pangu-input', value: draft.pb, placeholder: 'https://你的域名 或 http://IP:19529', onChange: (e) => setDraft((p) => ({ ...p, pb: e.target.value })), style: { width: '100%', boxSizing: 'border-box', padding: '7px 11px', borderRadius: 8, border: `1px solid ${css.border}`, background: css.bg2, color: css.t1, fontSize: 12, fontFamily: mono, outline: 'none' } }),
          '本机部署填 http://IP:19529 · 云端填地址。DSH 的 MCP 客户端共用此地址（重启 DSH 后重新挂载）。'),
        Field('盘古凭据', credSet ? '已配置' : '未配置', credSet ? css.ok : css.warn, '数据面与管理面共用',
          h('input', { className: 'pangu-input', type: 'password', value: draft.pk, placeholder: credSet ? (credHint || '已配置（掩码显示）· 输入新值覆盖') : '粘贴盘古凭据（安装时打印）', onChange: (e) => setDraft((p) => ({ ...p, pk: e.target.value })), style: { width: '100%', boxSizing: 'border-box', padding: '7px 11px', borderRadius: 8, border: `1px solid ${css.border}`, background: css.bg2, color: css.t1, fontSize: 12, fontFamily: mono, outline: 'none' } }),
          '灰字即当前生效的凭据（掩码，明文不回显）· 留空保持不变 · 同时用于 X-API-Key 数据面与 X-Admin-Key 管理面'),

        // ── 01 LLM ──
        SecHead('01', 'LLM 配置', '结晶 / 蒸馏 / 摘要用'),
        Field('提供商', null, null, '选择后自动带出默认端点与模型',
          h('div', null,
            h('select', { value: draft.provider, onChange: (e) => pickProvider(e.target.value), style: { width: '100%', padding: '7px 11px', borderRadius: 8, border: `1px solid ${css.border}`, background: css.bg2, color: css.t1, fontSize: 12, outline: 'none', cursor: 'pointer' } },
              LLM_PROVIDERS.map((p) => h('option', { key: p.id, value: p.id }, p.label)),
              h('option', { value: 'custom' }, '自定义 / 代理…')),
            h('div', { style: { display: 'flex', alignItems: 'center', gap: 6, marginTop: 6, fontSize: 10.5, color: css.t3 } },
              h('span', { style: { width: 6, height: 6, borderRadius: '50%', background: ACCENT, flexShrink: 0 } }),
              provider.url ? h('span', null, '默认端点 ', h('span', { style: { fontFamily: mono } }, provider.url), ' · 默认模型 ', h('span', { style: { fontFamily: mono } }, provider.model)) : '自定义端点 — 在下方填写 Base URL 与模型名')),
          null),
        h('div', { style: { display: 'grid', gridTemplateColumns: '1fr 1.6fr', gap: 10, padding: '12px 0', borderBottom: `1px solid ${css.borderSoft}` } },
          h('div', null,
            h(TextField, { key: 'model', label: '模型', value: draft.model, placeholder: provider.model || 'gpt-4o', onChange: (v) => setDraft((p) => ({ ...p, model: v })), mono: true })),
          h('div', null,
            h(TextField, { key: 'base', label: 'Base URL', value: draft.baseUrl, placeholder: provider.url || '', onChange: (v) => setDraft((p) => ({ ...p, baseUrl: v })), mono: true }))),
        h(TextField, {
          key: 'key', label: 'API Key', value: draft.apiKey, password: true, mono: true,
          desc: keySet ? `已配置（${keyHint || '已设置'}）。明文不回显，留空即保持原 Key 不变；填入新值则覆盖。` : '尚未配置。Key 仅写入本机（权限 600），不会回传到界面。',
          placeholder: keySet ? '留空保持不变' : '粘贴 API Key（Ollama 可留空）',
          onChange: (v) => { setKeyDirty(true); setDraft((p) => ({ ...p, apiKey: v })) },
        }),
        h('div', { style: { padding: '11px 0', borderBottom: `1px solid ${css.borderSoft}` } },
          h('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12 } },
            h('div', { style: { flex: 1, minWidth: 0 } },
              h('div', { style: { fontSize: 12.5, fontWeight: 500 } }, '连接验证'),
              h('div', { style: { fontSize: 10.5, color: css.t3, marginTop: 2 } }, '使用「已保存」的配置测试；刚改过请先保存')),
            h('button', { onClick: runTest, type: 'button', disabled: testState.s === 'testing', style: { flexShrink: 0, padding: '6px 14px', borderRadius: 7, border: `1px solid ${css.border}`, background: css.bg2, color: css.t2, fontSize: 12, cursor: testState.s === 'testing' ? 'default' : 'pointer' } }, testState.s === 'testing' ? '测试中…' : '测试连接'),
          ),
          testState.s === 'done' && h('div', { style: { display: 'flex', alignItems: 'center', gap: 8, marginTop: 8, border: `1px solid ${testState.ok ? css.ok : css.err}`, borderRadius: 8, padding: '7px 11px', fontSize: 11.5, background: withAlpha(testState.ok ? css.ok : css.err, '0d') } },
            ...
            h('button', { onClick: () => setTestState({ s: 'idle', msg: '', ok: false }), type: 'button', style: { marginLeft: 'auto', border: 'none', background: 'transparent', color: css.t3, fontSize: 10.5, cursor: 'pointer', flexShrink: 0 } }, '清除'),
          )),

        // ── 02 记忆维护 ──
        SecHead('02', '记忆维护', '巩固 · 转写 · 内容提取'),
        h(SettingRow, { key: 'ce', label: '自动巩固', desc: '合并相关记忆、衰减低价值记忆（03:00–05:00 窗口加权）' },
        ...
        h('div', { style: { padding: '11px 0', borderBottom: `1px solid ${css.borderSoft}` } },
          h('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 14 } },
            h('div', { style: { flex: 1, minWidth: 0 } },
              h('div', { style: { fontSize: 12.5, fontWeight: 500 } }, '巩固间隔'),
              h('div', { style: { fontSize: 10.5, color: css.t3, marginTop: 2 } }, '与标签页「记忆管线」联动')),
        ...
        // 02B 语音转写
        h('div', { style: { display: 'flex', alignItems: 'baseline', gap: 8, padding: '14px 0 7px' } },
          h('span', { style: { fontFamily: mono, fontSize: 9.5, fontWeight: 600, color: css.t3, border: `1px solid ${css.borderSoft}`, borderRadius: 4, padding: '0 6px' } }, '02B'),
          h('span', { style: { fontSize: 12, fontWeight: 600, color: css.t2 } }, '语音转写'),
          h('span', { style: { fontSize: 10.5, color: css.t3, marginLeft: 'auto' } }, 'Whisper · 默认关闭')),
        h(SettingRow, { key: 'whisper', label: '音频自动转写', desc: '音频记忆入库时转写为文本（base / small / medium）' },
          h(Toggle, { checked: draft.whisperEnabled, onChange: () => setDraft((p) => ({ ...p, whisperEnabled: !p.whisperEnabled })) })),
        draft.whisperEnabled && h(SettingRow, { key: 'whisper-m', label: '转写模型', desc: '越大越准也越慢' },
          h('select', { value: draft.whisperModel, onChange: (e) => setDraft((p) => ({ ...p, whisperModel: e.target.value })), style: { padding: '6px 10px', borderRadius: 7, border: `1px solid ${css.border}`, background: css.bg2, color: css.t1, fontSize: 12, outline: 'none' } },
            h('option', { value: 'base' }, 'base（推荐 · 平衡）'),
            h('option', { value: 'small' }, 'small（更准）'),
            h('option', { value: 'medium' }, 'medium（最准 · 慢）'))),
        // 02C 多模态
        h('div', { style: { display: 'flex', alignItems: 'baseline', gap: 8, padding: '14px 0 7px' } },
          h('span', { style: { fontFamily: mono, fontSize: 9.5, fontWeight: 600, color: css.t3, border: `1px solid ${css.borderSoft}`, borderRadius: 4, padding: '0 6px' } }, '02C'),
          h('span', { style: { fontSize: 12, fontWeight: 600, color: css.t2 } }, '多模态内容提取'),
          h('span', { style: { fontSize: 10.5, color: css.t3, marginLeft: 'auto' } }, '开关联动工具暴露面 · 默认关闭')),
        h(SettingRow, { key: 'mm', label: '文件 / 图片 / 视频提取', desc: '开启后 pangu_ingest_file 等工具进入暴露面，Agent 可把文件送进记忆；关闭时不可达（行为开关 + 暴露面联动）' },
          h(Toggle, { checked: draft.multimodal, onChange: () => setDraft((p) => ({ ...p, multimodal: !p.multimodal })) })),

        // ── 03 只读信息 ──
        SecHead('03', '只读信息', '动态查询 · 不可编辑'),
        h('div', { style: { paddingTop: 2 } },
          h(InfoRow, { label: '盘古服务', value: versions ? (isCloud ? '云端 ' : '本机 ') + 'v' + versions.server : '—' }),
        ...
        // ── 05 关于与更新 ──
        SecHead('05', '关于与更新', 'GitHub Releases · gh-proxy.org 加速'),
        h('div', { style: { border: `1px solid ${css.borderSoft}`, borderRadius: 10, padding: '12px 14px', marginTop: 9, background: css.bg1 } },
          h('div', { style: { display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' } },
            h('div', { style: { textAlign: 'center', minWidth: 80 } },
              h('div', { style: { fontSize: 9.5, color: css.t3 } }, '插件版本'),
              h('div', { style: { fontFamily: mono, fontSize: 15, fontWeight: 600, marginTop: 2 } }, versions ? 'v' + versions.plugin : '…'),
              h('div', { style: { fontSize: 9.5, color: css.t3 } }, 'dsh-pangu')),
            h('div', { style: { textAlign: 'center', minWidth: 80 } },
              h('div', { style: { fontSize: 9.5, color: css.t3 } }, '盘古服务'),
        ...
        // ── 吸底保存栏 ──
        h('div', { style: { position: 'sticky', bottom: 0, display: 'flex', alignItems: 'center', gap: 12, marginTop: 14, padding: '10px 14px', border: `1px solid ${css.borderSoft}`, borderRadius: 10, background: css.bg2 } },
          ...
          saveState.s === 'saved' && h('span', { style: { fontSize: 11.5, color: css.ok } }, saveState.msg || '已保存并生效'),
