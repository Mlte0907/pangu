...
results["env"] = {
    ...
    "machine": platform.machine(), "cpu_count": multiprocessing.cpu_count(),
}
...
try:
    ...
    samples = [
        "盘古是专业的记忆系统","Python 异步编程指南",
        "Kubernetes 集群部署","深度学习与自然语言处理",
        "RESTful API 设计原则",
    ]
    n_iters = 30
    ...
    results["sections"]["onnx"] = {
        "n_total": n_total, "elapsed_s": round(elapsed, 3),
    ...
    print(f"  text={n_total}, mean_ms={results['sections']['onnx']['ms_per_text']}, reported_avg={stats.get('avg_infer_ms')}")
    start = time.perf_counter()
    vecs = emb.embed_batch(samples * 4)
    elapsed_b = time.perf_counter() - start
    results["sections"]["onnx_batch"] = {
        "n_texts": len(samples*4), "elapsed_s": round(elapsed_b,3),
        "ms_per_text": round(1000*elapsed_b/len(samples*4), 3),
        "dim": len(vecs[0]) if vecs else 0,
    }
    print(f"  batch n={len(samples*4)}, ms/text={results['sections']['onnx_batch']['ms_per_text']}, dim={len(vecs[0])}")
...
try:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        results["sections"]["sqlite_cache"] = {
            "n_writes": n_writes, "write_s": round(write_t, 3),
            "write_us_each": round(1e6 * write_t / n_writes, 1),
            "n_reads": n_writes, "read_s": round(read_t, 3),
            "read_us_each": round(1e6 * read_t / n_writes, 1),
            "hits": hits, "hit_rate": round(hits/n_writes, 4),
            "total_entries": stats.get("total_entries"),
        }
        print(f"  write_us={results['sections']['sqlite_cache']['write_us_each']}, "
...
try:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        results["sections"]["memory_stack"] = {
            "n_writes": n_drawers, "write_s": round(w_t, 3),
            "write_ms_each": round(1000*w_t/n_drawers, 3),
            "list_s": round(l_t, 3), "list_count": len(ds),
        }
...
try:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        print(f"  threads={n_threads}, ops={n_threads*n_per_thread}, "
