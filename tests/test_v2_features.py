...
class TestNeuralMemory:
    ...
    def test_neural_sleep_consolidation(self):
        ...
        for i in range(3):
            engine.encode(
                Drawer(
                    id=f"test-{i}", content=f"记忆{i}", wing="test", importance=3.0, tags=["test"], created_at=old_time
                )
            )
    ...
    def test_neural_spreading(self):
        ...
        for i, content in enumerate(["Python", "ONNX", "FAISS"]):
            engine.encode(
                Drawer(id=f"test-{i}", content=content, wing="test", importance=3.0, tags=["test"], created_at=old_time)
            )
        engine.sleep()
...
class TestMultiAgent:
    ...
    def test_write_read(self):
        from pangu.memory.multi_agent import MemoryScope, get_multi_agent_memory
...
class TestTags:
    ...
    def test_tags_db_init(self):
        from pangu.api.routes_tags import _get_db, _init_db
