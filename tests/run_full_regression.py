...
def run_pytest(files, out_xml: str, extra_args: list = None) -> dict:
    ...
    t0 = time.time()
    r = subprocess.run(
        cmd, cwd=ROOT, capture_output=True, text=True, env={**os.environ, "PANGU_DATA_DIR": str(ROOT / ".test_data")}
    )
