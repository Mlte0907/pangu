...
class SelfEvolutionEngine:
    ...
    def diagnose(self, drawers: list, search_stats: dict = None,
                 lifecycle_stats: dict = None) -> list[DiagnosisResult]:
        """全面诊断系统健康状况"""
        results = []
        ...
        if total == 0:
            results.append(DiagnosisResult(
                ...
                metric_name="total_memories", metric_value=0,
            ))
        elif total < 10:
            results.append(DiagnosisResult(
                ...
                metric_name="total_memories", metric_value=total,
            ))
        ...
        if drawers:
            ...
            if avg_imp < 2.0:
                results.append(DiagnosisResult(
                    category="importance", severity="warning",
                    description=f"平均重要性偏低: {avg_imp:.2f}",
                    recommendation="调整重要性评分阈值，避免低质量记忆堆积",
                    metric_name="avg_importance", metric_value=avg_imp,
                    threshold=2.0,
                ))
            elif avg_imp > 4.5:
                results.append(DiagnosisResult(
                    category="importance", severity="info",
        ...
        if search_stats:
            ...
            if hit_rate < 0.3:
                results.append(DiagnosisResult(
                    category="search", severity="warning",
                    description=f"搜索命中率偏低: {hit_rate:.2%}",
                    recommendation="优化搜索参数，检查向量索引",
                    metric_name="search_hit_rate", metric_value=hit_rate,
                    threshold=0.3,
                ))
            if avg_score < 0.25:
                results.append(DiagnosisResult(
                    ...
                    metric_name="search_avg_score", metric_value=avg_score,
                    threshold=0.25,
                ))
        ...
        if lifecycle_stats:
            ...
            if pending_review > 50:
                results.append(DiagnosisResult(
                    category="lifecycle", severity="warning",
                    description=f"待复习记忆积压: {pending_review} 条",
                    recommendation="增加巩固频率或降低复习阈值",
                    metric_name="pending_review", metric_value=pending_review,
                    threshold=50,
                ))
        ...
        if dup_rate > 0.15:
            results.append(DiagnosisResult(
                category="dedup", severity="warning",
                description=f"重复记忆比例: {dup_rate:.2%} ({duplicates}/{total})",
                recommendation="提高去重阈值或运行去重清理",
                metric_name="duplicate_rate", metric_value=dup_rate,
                threshold=0.15,
            ))
