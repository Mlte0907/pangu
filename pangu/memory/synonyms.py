...
SYNONYM_MAP: dict[str, list[str]] = {
    ...
    # 代码/编程
    "代码": ["编程", "源码", "程序", "code", "编程代码"],
    "编程": ["代码", "开发", "coding", "写代码"],
    "开发": ["编程", "研发", "development", "构建"],
    "源码": ["代码", "源代码", "source code"],
    "函数": ["方法", "function", "接口", "API"],
    "变量": ["参数", "variable", "字段"],
    "调试": ["debug", "排错", "断点"],
    "重构": ["refactor", "重写", "改造"],
    "模块": ["组件", "module", "组件化"],
...
def expand_synonyms(keywords: str | list[str], max_expansions: int = 2) -> list[str]:
    """扩展同义词 — 兼容旧 str 调用与新 list 调用（t9 修复）

    Args:
        keywords: 分词后的关键词列表；兼容旧调用传入的原始查询字符串
                  （按空白分词，与 legacy 语义一致）。
        max_expansions: 每个词最大扩展数

    Returns:
        扩展后的同义词列表（含原词，保持既有测试/调用约定）
    """
    if isinstance(keywords, str):
        ...
        words = [w for w in keywords if isinstance(w, str)]
    ...
    for word in words:
        ...
        if word_lower in SYNONYM_MAP:
            expanded.update(SYNONYM_MAP[word_lower][:max_expansions])
        # 查找是否是某个词的同义词
        if word_lower in _REVERSE_MAP:
            original = _REVERSE_MAP[word_lower]
            for syn in SYNONYM_MAP.get(original, [])[:max_expansions]:
                if syn != word_lower:
                    expanded.add(syn)
