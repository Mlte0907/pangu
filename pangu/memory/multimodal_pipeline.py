...
class MultimodalPipeline:
    ...
    def ingest_text(
        ...
        if auto_store:
            ...
            result["stored"] = True
            result["memory_id"] = drawer.id
            result["drawer_id"] = drawer.id  # 与 pangu_add_memory 的 drawer_id 字段对齐
    ...
    def _store_memory(self, result: dict, wing: str) -> Drawer:
        """将提取结果存入 Palace 记忆（F1 修复：remember 后必须 add_drawer 真正落库，
        对齐 handle_add_memory 行为，否则 stored:true 但检索/删除不可见）"""
        ...
        # F1：真正持久化到 palace/drawers.json（与 handle_add_memory 的 add_drawer 路径一致）
        self.memory.add_drawer(drawer)
        return drawer

    @property
    def memory(self):
        """懒加载本管道对应的 MemoryStack（配置同源）"""
