...
class ConsolidationIntelligence:
    ...
    def find_merge_candidates(self, drawers: list[Drawer]) -> list[list[Drawer]]:
        """查找可合并的记忆组（跳过 Fernet 密文——密文共享标签但内容互异，标签聚类是误报）"""
        tag_groups: dict[str, list[Drawer]] = {}
        for d in drawers:
            if d.content.startswith("gAAAAAB"):
                continue
            # 防御：历史数据可能含重复 tag（如 ['fused','fused']），对同一 Drawer
            # 去重后再按 tag 入组，避免同一 id 在同一 tag 组重复出现。
            for tag in dict.fromkeys(d.tags):
        ...
        for tag, group in tag_groups.items():
            ...
            # 过滤已入选候选的 id —— 一条记忆带多个 tag 会出现在多个 tag 组，
            # 必须跨组去重，否则同一 id 会在不同组的候选里重复输出。
            group = [d for d in group if d.id not in seen_ids]
            if len(group) < 2:
                continue

            if len(group) >= 3:
                # 内容级校验：标签共现不等于内容冗余。仅当组内内容归一化前缀
                # 高度重叠（同源记录/重复写入）时才作为合并候选，否则保留为
                # 标签共现组（不产生假阳性合并建议）。
                def _norm(s: str) -> str:
                    import re
                ...
                sig: dict[str, list] = {}
                for d in group:
                    # 60 字符前缀：融合产物等内容常共享短开头（"关键点: [融合]…"），
                    # 短前缀会把不同正文误判为重复；真实重复记录整句归一化后前缀
                    # 会整体相同，60 字符能有效区分。
                    sig.setdefault(_norm(d.content)[:60], []).append(d)
                same_content = [g for g in sig.values() if len(g) > 1]
                if not same_content:
                    continue
                members = [d for g in same_content for d in g][:5]
                for m in members:
                    seen_ids.add(m.id)
                candidates.append(members[:5])
