...
class NaturalLanguageQuery:
    ...
    def _extract_time_range(self, query: str) -> timedelta | None:
        """提取时间范围"""
    ...
    def _extract_wing(self, query: str) -> str | None:
        """提取空间"""
...
class MemoryRecommender:
    ...
    def _fallback_recommend(self, limit: int) -> list[dict]:
        """降级推荐：返回最近的重要记忆"""
        sorted_drawers = sorted(self.drawers, key=lambda d: (d.importance, d.created_at), reverse=True)
        return [
            {
                "id": d.id,
                "content": d.content,
...
def _timeline_query(drawers: list[Drawer], parsed: dict, limit: int) -> list[dict]:
    ...
    if time_range:
        ...
        filtered = [d for d in drawers if datetime.fromisoformat(d.created_at) >= cutoff]
    else:
        filtered = drawers
