...
def conflicts(
                ...
                [
                    {
                        ...
                        "memory_a": c.memory_a,
                        "memory_b": c.memory_b,
                        "content_a": c.content_a[:100],
                        "content_b": c.content_b[:100],
    ...
    for c in conflicts:
        ...
        console.print(f"[{color}]●[/{color}] [bold]{c.severity.value.upper()}[/bold] (置信度: {c.confidence:.2f})")
...
def dedup(
    ...
    console.print(
        f"\n[bold]发现 {stats['duplicate_groups']} 组重复记忆, 共 {stats['total_duplicate_memories']} 条可合并[/bold]\n"
    )

    merged_count = 0
    for g in groups:
        console.print(
            f"[cyan]●[/cyan] 组 {g.id}: 主记忆={g.primary_id}, 重复={g.duplicate_ids}, 相似度={g.avg_similarity:.4f}"
        )
        if not dry_run:
...
def health():
    ...
    console.print(
        Panel.fit(
            ...
            + (
                "[建议]\n" + "\n".join(f"  - {r}" for r in analysis.recommendations) if analysis.recommendations else ""
            ),
            title="盘古健康检查",
        )
    )
...
def timeline(
    ...
    for e in events[:20]:
        table.add_row(e.timestamp[:16], f"{e.wing}/{e.room}", e.content[:80], f"{e.importance:.1f}")
            ...
            json.dumps(
                {
                    ...
                    "chains": [
                        {"id": c.id, "span": c.span, "summary": c.summary, "event_count": len(c.events)} for c in chains
                    ],
                },
                ensure_ascii=False,
                indent=2,
            ...
            json.dumps(
                {
                    ...
                    "insights": insights,
                    "patterns": [
                        {"id": p.id, "type": p.pattern_type, "description": p.description, "confidence": p.confidence}
            ...
            json.dumps(
                {
                    ...
                    "wings": session.wings,
                    "key_moments": [
                        {"time": m["time"][:16], "content": m["content"][:100]} for m in session.key_moments[:5]
                    ],
                },
                ensure_ascii=False,
                indent=2,
            ...
            json.dumps(
                {
                    ...
                    "title": session.title,
                    "highlights": [
                        {"time": m["time"][:16], "content": m["content"][:100], "importance": m["importance"]}
...
def adaptive_evaluate(
    ...
    if json_output:
        ...
        console.print(
            f"[{'green' if params.update_reason != 'no_change' else 'yellow'}]更新: {params.update_reason}[/]"
        )
...
def wm_get(
    ...
    if item_id:
        ...
        if item:
            console.print(
                json.dumps(
                    {"id": item.id, "content": item.content[:200], "activation": item.activation},
                    ensure_ascii=False,
                    indent=2,
                )
                if json_output
                else f"[{item.id[:16]}] 激活度={item.activation:.4f}: {item.content[:120]}"
            )
        else:
...
def wm_stats(json_output: bool = typer.Option(False, "--json", help="JSON 格式输出")):
    ...
    if json_output:
        ...
        console.print(
            f"容量: {stats['capacity']}, 使用: {stats['slots_used']}, "
...
def sanitize(
    ...
    if json_output:
        console.print(
            json.dumps(
                {"sanitized": sanitized, "redactions": redactions, "total": sum(redactions.values())},
                ensure_ascii=False,
                indent=2,
            )
        )
...
def attention(
    ...
    if stop_ab:
        ...
        console.print(
            json.dumps(result, ensure_ascii=False, indent=2) if json_output else f"胜者: {result.get('winner', 'N/A')}"
        )
        return
...
def trajectory(
    ...
    if json_output:
        ...
        console.print(f"事件: {result.get('total_events', 0)}, 回归: {result.get('regression_count', 0)}")
...
def privacy(
    ...
    if count is not None:
        ...
        if json_output:
            ...
            console.print(
                f"预算: {stats['remaining_budget']}/{stats['total_budget']} "
...
def system_health(
    ...
    if json_output:
        ...
        console.print(
            f"[{'green' if result['status'] == 'ok' else 'yellow'}]"
...
def llm_cache_stats():
    ...
    table.add_row(
        "累计 token",
        f"{stats.get('total_tokens')} ({stats.get('total_prompt_tokens')}+{stats.get('total_completion_tokens')})",
    )
...
def llm_cache_warmup(
    prompts_file: str = typer.Option(None, "--file", "-f", help="JSON 文件路径（含 prompts 列表）"),
