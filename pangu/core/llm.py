...
class LLMEngine:
    ...
    def client(self) -> httpx.AsyncClient:
        if self._client is None:
            # OpenCode Go 网关要求每个请求携带稳定的会话标识（x-opencode-session），
            # 缺失时请求会被拒绝。可用 PANGU_LLM_SESSION_ID 固定，否则进程内生成一个稳定 ID。
            session_id = os.environ.get("PANGU_LLM_SESSION_ID") or os.urandom(8).hex()
    ...
    async def _call_openai_compatible(
        ...
        if not api_key and provider != "ollama":
            if not self.config.llm_base_url:
                ...
                return LLMResponse(
                    content=(
                        f"[LMM 未配置 API Key，请设置 {env_key} 环境变量，"
                        f"或通过 pangu_config_set 配置 llm_base_url（本地/自托管 OpenAI 兼容端点，"
                        f"无需 key）后调用 pangu_config_reload 生效]"
                    ),
                    provider=provider,
                )
            # 已配置 llm_base_url 的本地/自托管 OpenAI 兼容端点：
            # 允许无 key 调用（下方不会附加 Authorization 头），避免「半死」状态
