...
TOOLS = [
    ...
    {
        "name": "pangu_event_chains",
        "description": "\u6784\u5efa\u4e8b\u4ef6\u94fe\uff08\u65f6\u95f4\u76f8\u8fd1\u7684\u4e8b\u4ef6\u5206\u7ec4\uff09",
    },
    ...
    {
        "name": "pangu_highlight_reel",
        "description": "\u63d0\u53d6\u6700\u91cd\u8981\u7684\u8bb0\u5fc6\u65f6\u523b\uff08\u7cbe\u5f69\u96c6\u9526\uff09",
    },
    ...
    return json.dumps(
        {
            ...
            "chains": [
                {"id": c.id, "span": c.span, "summary": c.summary, "event_count": len(c.events)} for c in chains[:10]
            ],
        },
        ensure_ascii=False,
        indent=2,
...
async def handle_timeline_query(server, drawers, arguments):
    ...
    return json.dumps(
        [
            {"id": e.drawer_id, "content": e.content[:150], "timestamp": e.timestamp, "wing": e.wing, "room": e.room}
            for e in result[:30]
        ],
        ensure_ascii=False,
        indent=2,
...
async def handle_temporal_timeline(server, drawers, arguments):
    ...
    return json.dumps(
        {
            "events": [
                {"id": e.memory_id, "content": e.content, "timestamp": e.timestamp, "wing": e.wing} for e in events
            ],
            "count": len(events),
        },
        ensure_ascii=False,
...
async def handle_temporal_relations(server, drawers, arguments):
    ...
    return json.dumps(
        {
            "relations": [
                {"before": r.before_id, "after": r.after_id, "relation": r.relation, "confidence": r.confidence}
                for r in rels
            ],
            "count": len(rels),
        },
    ...
    return json.dumps(
        {
            "links": [
                {
                    "cause": l.cause_text[:50],
                    "effect": l.effect_text[:50],
    ...
    return json.dumps(
        {
            "chains": [
                {
                    "id": c.chain_id,
                    "root": c.root_cause[:50],
                    "effect": c.final_effect[:50],
